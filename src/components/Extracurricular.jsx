...
const CertificationCard = ({ title, icon, type, date, points, credential }) => (
  <div className="certification-card bg-tertiary p-6 rounded-2xl w-full h-full flex flex-col justify-between no-select">
    <div>
      ...
      <h3 className="text-white font-bold text-[16px] sm:text-[18px] md:text-[20px] mb-2 no-select leading-tight">{title}</h3>
      <p className="text-secondary text-[11px] sm:text-[12px] mb-1 no-select">{type}</p>
      <p className="text-secondary text-[11px] sm:text-[12px] mb-3 no-select">{date}</p>
      <ul className="list-disc ml-4 sm:ml-5 space-y-1">
        {points.slice(0, 2).map((point, index) => (
          <li
            key={`certification-point-${index}`}
            className="text-white-100 text-[11px] sm:text-[12px] pl-1 tracking-wider no-select leading-relaxed"
          >
            {point}
          </li>
        ))}
    ...
    <div className="mt-4 flex justify-end no-select">
      <a
        ...
        className="black-gradient text-secondary py-1.5 px-3 sm:py-2 sm:px-4 rounded-lg outline-none w-fit text-[11px] sm:text-[12px] font-bold shadow-md shadow-primary transition-all hover:scale-105 hover:shadow-[0_0_10px_rgba(128,0,128,0.7)] no-select"
      >
        View Credential
      </a>
    </div>
